...
export default function ProductsError({ error }: ErrorProps) {
  ...
  return (
    <div className="w-full min-h-[75vh] flex flex-col items-center justify-center p-6 relative overflow-hidden bg-gradient-to-b from-slate-400 via-slate-300 to-zinc-400 rounded-2xl border border-slate-400 shadow-2xl">
      ...
      {/* СЛОЙ 2: ИДУЩИЙ СТЕНАМИ ДОЖДЬ (Падает наискось под наклоном ветра) */}
      ...
      {/* СЛОЙ 3: ИНФОРМАЦИОННОЕ ОКНО */}
      <div className="z-20 text-center max-w-sm bg-slate-900/20 backdrop-blur-xl border border-white/20 p-8 rounded-2xl shadow-2xl space-y-6">
        {/* Иконка непогоды */}
      ...
      {/* СЛОЙ 4: КАЧАЮЩИЕСЯ ОТ ВЕТРА ДЕРЕВЬЯ (Нижний план) */}
      <div className="absolute bottom-0 inset-x-0 h-28 flex items-end justify-between px-4 pointer-events-none z-0 select-none opacity-60">
        ...
        {/* Левый лес (3 дерева покачиваются вместе) */}
        <div className="flex items-end gap-0.5 origin-bottom animate-forest-wind">
          ...
          {/* Большая ель */}
          <div className="w-0 h-0 border-l-[25px] border-l-transparent border-r-[25px] border-r-transparent border-b-[75px] border-b-slate-800 -ml-5 relative z-10" />
          {/* Среднее дерево */}
